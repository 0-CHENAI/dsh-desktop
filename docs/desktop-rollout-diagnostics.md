...
## Update contract in the upstream project
...
The `0-CHENAI/dsh-desktop` fork intentionally does not wire this rollout endpoint into its update manager. Packaged and development checks continue to use this fork's GitHub Releases; historical-version feeds are pinned to the selected release archive, and mismatched metadata is rejected. The desktop service keeps the upstream policy client for source compatibility, but only diagnostics are initialized by this fork.
