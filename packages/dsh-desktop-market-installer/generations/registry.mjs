...
export async function sweepRegistry(dshHome) {
  ...
  let profileEntries = []
  try {
    profileEntries = await readdir(profiles)
  } catch {
    profileEntries = []
  }
  for (const name of profileEntries) {
    if (!/^\.market-validation-[A-Za-z0-9]{6}$/u.test(name)) continue
    try {
      await rm(join(profiles, name), { recursive: true, force: true })
      removed.push(`validation/${name}`)
    } catch {
