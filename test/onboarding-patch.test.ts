...
describe('desktop provider onboarding patch', () => {
  ...
  it('is captured as a reproducible dependency patch', async () => {
    ...
    expect(installed).not.toContain('SETTINGS_PROVIDER_PRIORITY')
    expect(installed).toContain('left.entry.displayName.localeCompare(right.entry.displayName, "en", { sensitivity: "base", numeric: true })')
    expect(installed).toContain('.sort(compareProviders)')
    expect(installed).toContain('providerIconUrl(row.entry.provider)')
    expect(installed).toContain('providerSupportsOAuth(row)')
    expect(installed).toContain('oauthProviderBadge: "OAuth"')
